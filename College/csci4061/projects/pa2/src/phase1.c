...
void phase1(char *filename, int m){
  ...
  // }
  //
  // // Quick string manipulation to redirect to Testcases folder
  // int len = strlen(cwd);
  // char * newdir = (char *)malloc(sizeof(char)*(len+7+strlen(filename)));
  // strncpy(&newdir[0], cwd, len-4);
  // char * testcases = (char *)"/Testcases/";
  // strcpy(&newdir[len-4], testcases);
  // strcpy(&newdir[len+7], filename);
