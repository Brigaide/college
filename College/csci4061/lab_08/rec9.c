...
void * partial_matrix_add(void * args) {
	args_t *nargs = (args_t *)args;
	for(int i = nargs->x_start; i < nargs->x_end; i++){
		for(int j = nargs->y_start; j < nargs->y_end; j++){
			nargs->res_mat[i][j] = mat1[i][j] + mat2[i][j];
		}
	}
	return nargs;
}
...
int main() {
    ...
    for(i = 0; i < 4; i++) {
		...
		m_args[i].x_end = 2500;
	    	...
	    	m_args[i].x_start =2500;
		m_args[i].x_end = 5000;
	    }
	    if(i == 0 || i == 1){
		    m_args[i].y_start = 2500;
		    m_args[i].y_end = 5000;		        
	    }
	    else{
	        m_args[i].y_start = 0;
